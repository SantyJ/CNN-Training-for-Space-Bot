...
# def plot_actual_vs_best_actions(metrics_log):
#     # Extract data from metrics_log
#     timestamps = [entry["timestamp"] for entry in metrics_log]
#     actual_actions = [entry["actual_action"] for entry in metrics_log]
#     best_actions = [entry["best_action"] for entry in metrics_log]
#     action_scores = [entry["action_scores"] for entry in metrics_log]

#     # Map actions to integers for plotting
#     action_mapping = {"up": 1, "down": 2, "left": 3, "right": 4, "sense": 5}
#     mapped_actual_actions = [action_mapping[action] for action in actual_actions]
#     mapped_best_actions = [action_mapping[action] for action in best_actions]

#     # Set bar width and positions
#     bar_width = 0.35
#     x_indexes = np.arange(len(timestamps))

#     # Create the plot
#     plt.figure(figsize=(12, 6))

#     # Plot actual actions
#     plt.bar(x_indexes - bar_width / 2, mapped_actual_actions, bar_width, label="Actual Actions", color="blue")

#     # Plot best actions
#     plt.bar(x_indexes + bar_width / 2, mapped_best_actions, bar_width, label="Best Actions (Model)", color="red")

#     # Add action scores as text at the top of the bars
#     for i, (score_dict, actual, best) in enumerate(zip(action_scores, mapped_actual_actions, mapped_best_actions)):
#         plt.text(x_indexes[i] - bar_width / 2, actual + 0.2, f"{score_dict.get(actual_actions[i], 0):.2f}", ha='center', fontsize=8, color="blue")
#         plt.text(x_indexes[i] + bar_width / 2, best + 0.2, f"{score_dict.get(best_actions[i], 0):.2f}", ha='center', fontsize=8, color="red")

#     # Customize the plot
#     plt.xlabel("Timestamps")
#     plt.ylabel("Actions (Mapped to Numbers)")
#     plt.title("Actual vs Best Actions Over Time")
#     plt.xticks(ticks=x_indexes, labels=timestamps, rotation=45, fontsize=8)
#     plt.yticks(ticks=range(1, 6), labels=list(action_mapping.keys()))
#     plt.legend()
#     plt.tight_layout()

#     # Save and show the plot
#     plt.savefig("actual_vs_best_actions.png")
#     print("Plot saved as actual_vs_best_actions.png")
#     plt.show()

def plot_actual_vs_best_actions(metrics_log):
    # Extract relevant data
    timestamps = [entry["timestamp"] for entry in metrics_log][-10:]
    actual_actions = [entry["actual_action"] for entry in metrics_log][-10:]
    best_actions = [entry["best_action"] for entry in metrics_log][-10:]
    action_scores = [entry["action_scores"] for entry in metrics_log][-10:]

    # Mapping actions to integers for easier visualization
    action_mapping = {"up": 1, "down": 2, "left": 3, "right": 4, "sense": 5}
    actual_action_vals = [action_mapping[action] for action in actual_actions]
    best_action_vals = [action_mapping[action] for action in best_actions]

    # Bar width for side-by-side comparison
    bar_width = 0.4
    x = np.arange(len(timestamps))  # X-axis positions for timestamps

    # Create the bar plot
    plt.figure(figsize=(12, 6))
    plt.bar(x - bar_width / 2, actual_action_vals, bar_width, label="Actual Actions", color="blue")
    plt.bar(x + bar_width / 2, best_action_vals, bar_width, label="Best Actions", color="red")

    # Add action scores above bars
    for i in range(len(timestamps)):
        actual_score = action_scores[i][actual_actions[i]]
        best_score = action_scores[i][best_actions[i]]
        plt.text(x[i] - bar_width / 2, actual_action_vals[i] + 0.1, f"{actual_score:.2f}", ha="center", color="blue", fontsize=9)
        plt.text(x[i] + bar_width / 2, best_action_vals[i] + 0.1, f"{best_score:.2f}", ha="center", color="red", fontsize=9)

    # Add labels and title
    plt.xlabel("Timestamps")
    plt.ylabel("Actions (Mapped)")
    plt.title("Actual vs Best Actions (Last 10 Timestamps)")
    plt.xticks(x, timestamps, rotation=45)
    plt.yticks(list(action_mapping.values()), list(action_mapping.keys()))
    plt.legend()
    plt.tight_layout()

    # Save the plot
    filename = "actual_vs_best_actions_last_15.png"
    plt.savefig(filename)
...
def main_phase_2(grid, initial_bot_position, rat_initial_position, alpha, state, data_log, model):
    ...
    while state.movements < timeout_limit:
        ...
        # Check if a saved model exists
        model_path = "space_rat_model_dynamic.pth"
        if os.path.exists(model_path):
            print("Loading saved model...")
            model.load_state_dict(torch.load(model_path))
            model.eval()
    ...
    if os.path.exists(model_path):
        ...
        # Example usage: Call this function at the end of a simulation
        # plot_actual_vs_best_actions(metrics_log)
...
def calculate_average_agreement(all_simulations_data):
    ...
    valid_simulations = [sim_data for sim_data in all_simulations_data if sim_data and "agreement_percentage" in sim_data]
    
    # Extract agreement percentages from valid simulations
    agreement_percentages = [sim_data["agreement_percentage"] for sim_data in valid_simulations]
    
    # Calculate average
    if len(agreement_percentages) > 0:
        avg_agreement = sum(agreement_percentages) / len(agreement_percentages)
    else:
        avg_agreement = 0  # Handle case where there are no valid simulations
